    ...
    async syncFromContifico(ctx) {
        try {
            ...
            for (const p of productosExternos) {
                const categoria = await strapi.entityService.findMany('api::categoria.categoria', {
                    filters: { ID_Categoria: p.categoria_id },
                });

                const existente = await strapi.entityService.findMany('api::producto.producto', {
                ...
                if (existente.length > 0) { 
                    ...
                    await strapi.entityService.update('api::producto.producto', 
                        existente[0].id, { 
                            data, 
                        }); 
                }else { 
            ...
            ctx.body = { error: 'Error al sincronizar productos', details: err.message };
        }
    },
}));
